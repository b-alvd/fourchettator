...
async function hydrate(row) {
  const [ing, steps] = await Promise.all([
    query("SELECT name, qty, unit FROM ingredients WHERE recipe_id = ? ORDER BY position", [row.id]),
  ...
  return {
    ...
    blurb: row.blurb,
    ing: ing.map((i) => [i.name, Number(i.qty), i.unit]),
    steps: steps.map((s) => ({
      content: s.content,
      section: s.section || "",
...
async function insertChildren(id, r) {
  for (let i = 0; i < r.ing.length; i++) {
    ...
    await query("INSERT INTO ingredients (recipe_id,position,name,qty,unit) VALUES (?,?,?,?,?)", [id, i, name, Number(qty) || 0, unit || ""]);
  }
