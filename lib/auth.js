...
/* ---------- utilisateur courant (à partir du cookie) ---------- */
export async function getCurrentUser() {
  ...
  if (!u) return null;
  if (Number(u.session_version || 0) !== Number(sess.sv || 0)) return null; // session invalidée
...
/* ---------- désabonnement marketing (lien signé, sans login) ---------- */
...
/* ---------- rôle admin (emails listés dans ADMIN_EMAILS) ---------- */
