...
export default function RatingStars({ recipeId, avg, votes, mine }) {
  const { user } = useAuth();
  const router = useRouter();
  const [rating, setRating] = useState(avg || 0);
  const [count, setCount] = useState(votes || 0);
  const [myVote, setMyVote] = useState(mine || 0);
  ...
  return (
    <div className="rating">
      <div className="rating-stars" onMouseLeave={() => setHover(0)}>
        {[1, 2, 3, 4, 5].map((n) => (
          <button key={n} className={`star ${n <= fillTo ? "on" : ""}`}
            onMouseEnter={() => setHover(n)} onClick={() => rate(n)} aria-label={`Noter ${n} sur 5`}>★</button>
        ))}
      </div>
